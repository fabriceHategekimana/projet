...
print("Var value is: ", np.var(rand2d))# Row wise and column wise mean
...
![plt.xlabel('x')](images/plt.xlabel('x').png)
...
def cos_product(x,N):
    ...
    ####### ADD YOUR CODE# Verify your function using values for which you know the result. For example, $\cos(0) = 1$ , $\cos(\pi) = -1$ 
cos_product(0,10)
