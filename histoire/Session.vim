...
set ruler
set runtimepath=~/.vim,~/.vim/pack/tpope/start/surround,~/.vim/plugged/swift.vim/,~/.vim/plugged/nerdtree/,~/.vim/plugged/vimwiki/,~/.vim/plugged/syntastic/,~/.vim/plugged/hardmode/,~/.vim/plugged/lf.vim/,/var/lib/vim/addons,/etc/vim,/usr/share/vim/vimfiles,/usr/share/vim/vim82,/usr/share/vim/vimfiles/after,/etc/vim/after,/var/lib/vim/addons/after,~/.vim/after
set smartcase
set spelllang=fr_ch,en_us
set splitbelow
set splitright
set statusline=%#Cursor#\ VIM\ \ %#Normal#%{(mode()=='n')?'\ \ NORMAL\ ':''}%#PmenuThumb#%{(mode()=='i')?'\ \ INSERT\ ':''}%#TabLineFill#%{(mode()=='r')?'\ \ REPLACE\ ':''}%#SpellRare#%{(mode()=='v')?'\ \ VISUAL\ ':''}%#LineNr#\ %n\ %#Visual#%#CursorIM#%R%M%#Cursor#%#CursorLine#\ %t\ %=%#LineNr#\ %3l:%-2c\ %#Normal#\ %Y\ %#Cursor#%3p%%\ %#warningmsg#%{SyntasticStatuslineFlag()}%*
...
%argdel
$argadd note/note.md
edit ~/projet/histoire/note/Thèmes.md
...
tabnext 1
badd +7 note/note.md
badd +0 ~/projet/histoire/note/Thèmes.md
